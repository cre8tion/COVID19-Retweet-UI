...
def train(
    ...
    params = {"tree_method": "approx", "booster": booster}
    model = None
    stats = []
    ...
    if load_checkpoint:
        ...
        if len(past_stats) != start_ep - 1:
            raise ValueError(
                f"len(past_stats) is {len(past_stats)} but should be start_ep-1, which is {start_ep - 1}"
            )
        model = xgb.Booster()
...
def predict(model: xgb.Booster, x: np.ndarray, feature_names: list = None):
