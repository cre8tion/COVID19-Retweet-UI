...
class Item(object):
    ...
    def __init__(self, id, username, timestamp, followers, friends, favourites, mentions_counts, hashtag_counts, url_counts, retweets, predicted_retweets, model=0):
        self.id = id
    ...
    def get_elements(cls, model):
        if (model == 0 or model == 1 or model == 2):
            items_list = [Item(i['id'], i['username'], i['timestamp'], i['followers'], i['friends'], i['favourites'], 
            i['mentions_counts'], i['hashtag_counts'], i['url_counts'], i['retweets'], 
            i['predicted_retweets'], i['model']) for i in load_data(model)]
            return items_list
