...
def load_data(model_number):
    ...
    for i in range(len(data_dict)):
        item = {"id": data_dict[i]['index'], "username":data_dict[i]['Username'], "followers": data_dict[i]['#Followers'],
        "favourites": data_dict[i]['#Favorites'], "friends": data_dict[i]['#Friends'], "retweets": data_dict[i]['#Retweets'],
        "timestamp": data_dict[i]['Timestamp'], "mentions_counts": data_pred_dict[i]['Mentions_count'], 
        "hashtag_counts": data_pred_dict[i]['Hashtag_counts'], "url_counts": data_pred_dict[i]['URL_counts'],
        "predicted_retweets": int(prediction[i]), "model": model_number}
